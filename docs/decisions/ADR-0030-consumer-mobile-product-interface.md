...
## Amendment 2026-08-26: Phase 8 native depth pass

The product-owner Phase 8 brief asks NIGHT CURRENT to feel native rather than like the browser surface scaled down. Mobile therefore draws low-cost atmospheric fields from the same Consumer washes through the SVG module that already renders every product icon, gives the active tab a visible thumb-scale enclosure, and separates the native header from scrolling content. The fields are non-interactive and hidden from accessibility; no native dependency, palette value, type step, navigation destination, business state, or permission was added. Safe-area handling, dynamic text, Android system bars, keyboard behavior, and the existing Consumer design-parity gate remain authoritative.
