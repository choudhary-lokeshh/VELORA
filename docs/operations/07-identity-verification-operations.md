...
Current implementation provides the verified callback inbox, bounded lease/retry/dead-letter worker, redacted failure logging, transactional evidence/outbox write, a bounded provider-truth reconciliation worker, and two read-only Admin projections. The aggregate projection is identifier-free; the exact-subject projection accepts only an already-known opaque owner pair and consumes a one-time ADR-0017 action bound to that subject and executing Admin session. Reconciliation advances a technical due marker in PostgreSQL before its provider read, records normalized findings before repair, and uses the same append-only evidence/outbox path as callback processing. It records rather than autonomously starts a pre-provider attempt; it never retries a provider create, infers a result on retrieval failure, performs a manual repair, or carries out a privacy operation. Alerts/dashboards, privileged repair, and privacy-obligation execution remain unavailable; the two reads do not create a general-purpose Admin identity lookup or permission.
