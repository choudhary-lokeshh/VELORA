...
| Evidence resurrection or history rewrite | Immutable evidence, one-successor chain, one root per subject and class so a subject cannot hold two current tips, expected current tip, validity/expiry constraints | Trigger/constraint, second-root refusal, and race tests |
| One class answers for another, or a lapsed fact answers at all | Readers resolve by owner domain, exact owner reference, purpose, and evidence class; a grant is not current at the instant it expires; a revocation reads as a revocation; a later attempt outranks an earlier fact whatever timestamp a provider stamped on it | Adversarial cross-class, expiry-boundary, revocation, delayed-success, and in-flight re-verification tests |
