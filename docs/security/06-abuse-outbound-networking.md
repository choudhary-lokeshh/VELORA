...
Identity adapters inherit the same rule. Provider API origins are fixed approved configuration and never taken from a route, callback, redirect, provider payload, hosted URL, or evidence reference. A redirect target is not fetched. Any provider-returned URL used for retrieval must be a capability-specific adapter field validated against that provider's exact allowlist and cannot be forwarded to general outbound retrieval. The provider-specific ADR owns required egress destinations, DNS/IP behavior, credentials, timeouts, byte limits, and negative SSRF tests.
