...
`/v1/creator/offers` and the three operations beside it are Creator Studio only, on the same audience rule every other creator route follows. The acting creator comes from the session; no schema in the commercial contract names a creator, so a request cannot address somebody else's terms whatever it carries.

An amount crosses the wire as a decimal string of minor units beside its currency, never as a JSON number. JSON's only numeric type is a double, and a price is the last value that may quietly lose precision on the way to a database.

Three refusals are deliberately shaped. A creator who may not operate, a resource that is absent or not theirs or not published, and an amount outside approved terms all answer `403` — telling them apart would let one creator probe another's catalog one identifier at a time. A concurrent change, a live offer that already covers the resource, and a live price that already exists in the currency all answer `409 STATE_CONFLICT`, because in every case the caller should re-read and decide again rather than retry. And an environment with no approved commercial terms answers `503 DEPENDENCY_UNAVAILABLE`, which is a statement about the platform rather than about the caller; it carries no `Retry-After`, because there is nothing to come back to until somebody approves terms. The read is exempt from all of this: it answers `200` with an explicit readiness statement whether or not anything is sellable.

## Identity Assurance core surface

The V1 Identity contract scope is limited to `POST /v1/identity/provider-events`, `GET /v1/admin/identity/state`, and `GET /v1/admin/identity/subject`. All three routes are implemented. The callback consumes bounded raw bytes so provider authenticity can be verified before parsing; verified first and duplicate receipts return `202`, authentication failure is opaque `401`, oversized bodies return `413`, and an unconfigured provider returns `503 DEPENDENCY_UNAVAILABLE`. No callback body is returned or persisted.

Admin state is aggregate and identifier-free. Subject read accepts precisely one opaque owner pair, rejects unknown or repeated query fields, and has no list/search/export/mutation counterpart. It requires Platform Admin audience plus a current, single-use ADR-0017 exact action bound to the operation, subject, and executing Admin session. There is no public contract that issues this action, and consuming it does not resolve the granular Admin permission-matrix decision. Consumer and Creator initiation/read routes are absent until Phase 2; commercial-KYC/payout-readiness routes are absent until Phase 3. Provider, assurance, workflow, and policy selector fields are absent from every client contract.
