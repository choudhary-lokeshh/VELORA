...
The [ADR-0033](../decisions/ADR-0033-local-test-ai-suggestion-platform.md) baseline now provides the repository-owned deterministic adapter and real PostgreSQL regression layer for registered model/task routing, admission, redaction, immutable evidence, version pins, cross-caller isolation, duplicate identity, retry, provider failure, timeout, malformed/oversized input/output, inert action-shaped output, cancellation ownership, and quota. Four surface suites verify review/edit control and absence of implicit effects. Real browser journeys cover bio Save, explicit chat Send, and Creator caption Publish through the same adapter and API. These tests are conformance and safety evidence; they do not substitute for provider quality, portability, human-label, cost, latency, or production release thresholds.
