...
Owners authorize every start/read. Consumer and creator credentials can access only their own later-phase routes. Platform Admin receives exact-reference read and aggregate health only. The exact read additionally consumes the current, single-use ADR-0017 authorization bound to `identity.read_subject`, that opaque subject, and the executing Admin session; it is not a substitute for the unresolved granular Admin permission matrix. Manual grant, override, provider-dashboard decision, force-retry, evidence revocation, or deletion is not exposed until a separate legal/product/security decision defines actor, evidence standard, exact action, approval, expiry, appeal, and audit under ADR-0017.
