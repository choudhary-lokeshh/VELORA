...
Virtual gifts use the same financial path with one deliberate branch. BILLING first establishes the sender-scoped gift operation and links the checkout operation; only a verified provider settlement may advance it to `sent`, and that transaction posts the balanced capture and creator-revenue fact. It then stops before subscription or entitlement publication because a gift grants no access. Verified payment failure moves a pending gift to `failed`; partial or full refund and lost-dispute facts advance the gift reversal state without editing the capture. See [ADR-0032](../decisions/ADR-0032-provider-neutral-virtual-gifting.md).
...
Phase 2 first commercial releases; Phase 3 coins/gifts/payout impact. Balanced owner-specific journal architecture is locked by [ADR-0011](../decisions/ADR-0011-payments-payouts.md) and implemented under [ADR-0021](../decisions/ADR-0021-monetization-money-architecture.md); [money flow](../architecture/10-money-flow.md) draws the whole path including reversal and payout. No payment provider is eligible today, evidenced in [provider eligibility](../compliance/06-payment-provider-eligibility.md), so every state below is reachable only where configuration permits a test adapter — local and test. `DECISION REQUIRED / LEGAL REVIEW REQUIRED`: capture ordering per provider/product, refunds, dispute/chargeback handling, tax, journal posting rules, accounting mappings, and recognition/reporting policy. See [BILLING](../domains/billing.md), [payment security](../security/05-payments-webhooks.md), [payment/payout gates](../compliance/04-payments-tax-payout-gates.md), [finance operations](../operations/03-finance-payout-operations.md), [jobs](../engineering/03-jobs-idempotency-concurrency.md).
