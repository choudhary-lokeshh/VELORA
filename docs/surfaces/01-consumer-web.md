...
The implemented information architecture is locked by [ADR-0027](../decisions/ADR-0027-consumer-web-product-interface.md) and extended by [ADR-0040](../decisions/ADR-0040-random-live-discovery.md): six consumer destinations — Live, Discover, Introductions, Messages, Notices, You — each at its own address, with availability, safety, memberships, and settings under You. Live is first, and an admitted person lands there, because meeting one person who is here now is the reason to open the product; Discover is browsing and is one press away. Calling is still deliberately not a destination, because a call is placed against a mutual introduction and against nothing else; a navigation item per backend module would be exactly the architecture leak this document forbids. One navigation model has three arrangements — a bottom bar within thumb reach below the tablet breakpoint, a labelled rail above it, a persistent sidebar on desktop — and a conversation is an address rather than a pane, so browser Back leaves it.

Messages is a list beside the selected thread on desktop and a dedicated address on a narrow viewport. Each list row renders the counterpart's authorized avatar and name, the server's bounded newest-message preview, last activity, and a non-numeric unread mark derived from the caller's two sequence positions. The thread renders server-sequenced history, truthful sending/failed/refused states with idempotent retry, monotonic read progression, block/report, and voice/video call entry bound to the published mutual-introduction relationship. The composer is explicitly text only: attachments, typing, and presence are absent rather than simulated, and the surface states that messages are not end-to-end encrypted.

The Phase 3-shaped local/test proof in [ADR-0033](../decisions/ADR-0033-local-test-ai-suggestion-platform.md) adds labeled profile-bio and conversation-draft assistance. Generate/Regenerate and Cancel belong to the assistant; its result stays editable and reaches the real profile or message form only through **Replace draft**. Save and Send remain separate existing controls, so generation performs no USERS or MESSAGING effect. The deployed API keeps the provider unavailable and kill switch enabled; this surface reports that refusal and does not claim a weaker success.
...
A person is an address of their own, reached from the portrait on a card. It shows every photograph they added rather than the first, and the bio unclamped, and nothing else the projection did not already carry. Somebody nobody may see and somebody who is not there produce the same screen, because they produce the same answer.
