...
The Phase 3-shaped local/test proof in [ADR-0033](../decisions/ADR-0033-local-test-ai-suggestion-platform.md) offers labeled, tone-selectable bio, title, caption, description, content-idea, and club-announcement drafts. Every result is editable and must be explicitly copied into its adjacent draft. Save and Publish remain different existing decisions. Club announcements use a scratchpad that says it is neither saved nor published, because this milestone has no announcement domain object and the assistant cannot invent one.
