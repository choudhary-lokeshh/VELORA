...
Home reports what the creator actually has: drafts, published items, published clubs, and the real number of people who currently hold club access, each counted from a list the server returned in the same read and labelled as describing the pages that arrived rather than a total nobody computed. It offers exactly one next step, chosen from server state — no page, an unpublished page, a draft waiting, no club. There is no earnings figure, growth line, conversion rate, view count, follower count, subscriber count, or trend, because none of those exists as platform truth; a browser assertion walks every destination and fails on any of those words.
...
Money is three honest readouts. Earnings shows one currency at a time and never a total across currencies, rendered against each currency's published minor-unit exponent. Payouts separates a missing provider from a missing recipient record, shows the balances anyway because the money is real whatever the platform can do with it, and carries one idempotency key across every attempt at one payout intent. Selling states that nothing can be sold and offers no price field, because both offer operations refuse in every deployed environment and a form that always fails is worse than an explanation.
...
Three capabilities are deliberately absent and say so in their own words. No image can be added to a profile or an item, because the contract publishes no creator media route. There is no notification control anywhere, because the notification contract refuses a Creator Studio credential outright. Mature content is refused for four reasons, each attributed to somebody other than the creator reading them. All three are recorded in [DECISIONS_REQUIRED](../decisions/DECISIONS_REQUIRED.md).
