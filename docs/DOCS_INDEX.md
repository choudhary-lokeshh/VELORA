...
| [Virtual gifting freeze report](architecture/23-virtual-gifting-freeze-report.md) | What is runnable end to end in local/test, the gift/catalog/payment/journal/reversal and surface boundaries, and every production provider, legal, product, design, and payout block that remains |
| [Whole-product QA freeze report](architecture/24-whole-product-qa-report.md) | Phase 10 browser, Android-emulator, seeded-world, API-inventory, runtime-cleanliness, and intentional-blocker evidence before AI implementation begins |
| [AI suggestion platform freeze report](architecture/25-ai-platform-freeze-report.md) | What the local/test AI gateway and its draft-only Consumer, Creator, and Admin surfaces froze with, the hostile PostgreSQL and real-device evidence behind them, the forensic record of the phases this work builds on, and every live-provider decision that remains open |
| [Data integrity audit](architecture/26-data-integrity-audit.md) | Every number, state and claim on all four surfaces classified as real runtime data, legitimate seeded data, an honest unavailable state, or a fabrication — and the three fabrications found and fixed: memberships counted as people, one page of introductions rendered as three totals, and a capped queue printed as the whole queue |
| [Security correctness audit](architecture/27-security-correctness-audit.md) | Consumer, creator, admin, sensitive-action, web/API and auditability boundaries checked against their controls: no authorization hole found, and three guarantees that were held by construction and enforced by nothing now proved — the contract/CORS header agreement, consumer financial read isolation, and an admin route sweep that spoke for the whole surface while probing a quarter of it |
| [Financial correctness audit](architecture/28-financial-correctness-audit.md) | Payment, membership, gift, ledger, payout and tax invariants checked against the tests that hold them: the matrix was already complete, and the one gap closed was `partially_reversed` — a gift state the contract publishes, the database constrains, two services transition to, three surfaces render, and no test had ever produced |
| [Operational readiness audit](architecture/29-operational-readiness-audit.md) | Whether an important product failure is visible: one silent class found and fixed — the outbox relay and both provider-event drains retired work permanently and logged nothing, because `Poller` reports a cycle that throws and a dead letter does not throw — plus the bounded-retry, redaction, health, and queue-health findings that already held |
| [Test infrastructure audit](architecture/30-test-infrastructure-audit.md) | Every recorded CI hazard classified as a production defect, a fixture defect, a CI defect or a local one, and treated accordingly: one clock per lifecycle with a ratchet holding the rest, a connection ceiling that counts what the suites ask for, an override that cannot outlive its catalog entry, the push-registration 500 investigated to a captured SQLSTATE and left unfixed because its root cause is still unproven, and dangling volumes attributed but deliberately not deleted |
| [Whole-product runtime QA](architecture/31-whole-product-runtime-qa.md) | A fresh stack walked in a real browser rather than a built fixture: six services, seventy migrations, a seed proved idempotent by measurement, all three web surfaces including privileged admin access, five widths and 200 % text with no overflow anywhere, and no product defect found — with the Android journey recorded as not re-walked, because the emulator cannot boot on a machine with 209 MB free |
| [Production enablement readiness](architecture/32-production-enablement-readiness.md) | Eleven external capabilities, each with its port, adapters, configuration key, webhook endpoint, failure behaviour and the one human decision it waits on — every one interface-ready and provider-blocked, with no activation claimed and none possible until staging can boot at all |
| [Final release gate](architecture/33-final-release-gate.md) | Whether VELORA can truthfully be declared production-ready, and the answer: no, for reasons entirely external — each provider classified as blocked on a credential, a provider, a policy or a country rather than lumped together, with the AUTH signing authority named as the first gate because staging and production cannot start without it |
...
| REALTIME | [REALTIME](domains/realtime.md) | RTC flow, RTC threat model, RTC provider eligibility, provider adapters, Trust & Safety, ADR-0008, ADR-0025, ADR-0040, ADR-0043 |
| LIVE | [LIVE](domains/live.md) | random live discovery, REALTIME, DISCOVERY, MESSAGING, WALLET, Trust & Safety, ADR-0025, ADR-0040, ADR-0041, ADR-0042, ADR-0043, ADR-0044 |
| WALLET | [WALLET](domains/wallet.md) | coin ledger, paid matching preferences, coin packs, BILLING, LIVE, Consumer Web, Consumer Mobile, ADR-0011, ADR-0043, ADR-0044 |
...
| GROWTH | [GROWTH](domains/growth.md) | public entry and SEO, ADR-0047, privacy, USERS (attribution is applied only where an account is created), Consumer Web/Mobile |
| OPERATIONS | [OPERATIONS](domains/operations.md) | operator runbooks, ADR-0048, ADMIN, AUTH (assurance still comes first), LIVE and GROWTH (the paths a control governs) |
...
| [Public entry and SEO](engineering/08-public-entry-and-seo.md) | Which addresses are offered to a search engine and which are refused, the two conditions indexing requires, canonical and sitemap rules, what a social preview may carry, and the external steps only the owner can take |
| [Operator runbooks](engineering/09-operator-runbooks.md) | What to do when something is wrong, written against controls and screens that exist: a live abuse spike, invitation abuse, a compromised account, an RTC outage, failing payments, a worker backlog, push failures, a safety or support backlog, an operator whose access is wrong, and nothing appearing to be indexed |
...
| [ADR-0032](decisions/ADR-0032-provider-neutral-virtual-gifting.md) | BILLING-owned provider-neutral gifts on the ADR-0021 money path; published creator-profile recipient resolution, pair re-authorization, catalog/price authority, no entitlement, minimized histories, local/test-only settlement, and production fail-closed gates |
| [ADR-0033](decisions/ADR-0033-local-test-ai-suggestion-platform.md) | Provider-neutral synchronous AI suggestion gateway; local/test deterministic adapter; version-pinned PostgreSQL run/budget evidence; draft-only Consumer/Creator and bounded review-only Admin surfaces; production fail-closed |
| [ADR-0034](decisions/ADR-0034-local-test-admin-authenticator.md) | Local-test privileged authenticator verifier adapter for Platform Admin development access; loopback-only issuance route with phishing-resistant assurance; staging/production fail-closed |
| [ADR-0035](decisions/ADR-0035-club-membership-product.md) | The club membership product on the ADR-0021 money path; one live price per offer/currency/cadence with the purchase naming the cadence, consumer cancellation that schedules rather than takes, period-end expiry through the outbox, no invented grace period, provenance-aware departure, club benefits as PRIVATE CLUBS presentation, a club destination that answers an empty feed to anybody it does not admit, and no mobile purchase path |
| [ADR-0036](decisions/ADR-0036-platform-admin-operations-console.md) | Seven ADMIN operator reads and no new operation; an overview counted by the platform rather than by the console; accounts as an enforcement work list bounded by the platform's own decisions with no name, handle, or contact detail; a payment that carries no payer and a payout that carries no destination; both audit records read under the destination that owns them; and the console driven in a real browser through the ADR-0034 door |
| [ADR-0037](decisions/ADR-0037-consumer-web-experience-refinements.md) | Consumer Web experience refinements under ADR-0027: the page name handed between the heading and the bar rather than printed twice on a phone, a Back named where the navigation names its target, nothing overflowing at 200 % text, a section switch that behaves as `role="tablist"` promises, the introductions group as an address defaulting to where the work is, a `Section` primitive, a gift history that draws the gift, a refusal that names its gate wherever it is made, an unsent message with one copy and one safe way to send it, and the writing assist folded away inside a conversation |
| [ADR-0038](decisions/ADR-0038-creator-studio-workspace-refinements.md) | Creator Studio workspace refinements under ADR-0028: a tab bar that keeps all five destinations on a 320 px screen at 200 % text, a button, a badge and a row that no longer push the page sideways, the 200 %-text assertion re-applied after each navigation so it can fail at all, the page name handed between the heading and the bar rather than printed twice on a phone, a Back named where the navigation names its target, the catalog slice as an address that survives a reload, a filter strip that behaves as `role="tablist"` promises and names the region it changes, and a received-gift row that draws the gift, dates it like every other date, and tells a failure apart from a settlement |
| [ADR-0039](decisions/ADR-0039-consumer-mobile-device-refinements.md) | Consumer Mobile refinements under ADR-0030, found on an Android device rather than in a browser: the development server made to serve a bundle in this workspace, an SVG atmosphere that keeps its alpha instead of painting at full opacity, two screens that stop asserting no photograph is displayed while displaying them, one list of the sections under You so a real address stops being refused by the deep-link parser, a person at their own address carrying every photograph, a history of gifts sent with no send control and no outside link, gift silhouettes held together across three surfaces by the parity gate, a row of controls that becomes a column at large text, and a pushed screen that holds the system gesture band open itself |
| [ADR-0040](decisions/ADR-0040-random-live-discovery.md) | Random live discovery and the LIVE domain that owns it: a matching pool serialized by one global lock so nobody is handed to two people, an RTC session that carries a purpose and is re-authorized against the relationship it was created under, Connect as DISCOVERY's own introduction so a person met live and a person met in Discover are one kind of connection in one Inbox, in-encounter chat that is durable for moderation and never becomes Inbox history, presence measured by reading rather than announced, no count of anybody published anywhere, a block that ends the encounter and the session together under one pair lock, a stand-in that is a real seeded account driven through the published service methods, and Live as the primary consumer destination on both surfaces — refused outright in every deployed environment |
| [ADR-0041](decisions/ADR-0041-live-discovery-preferences-choice-and-presence.md) | Live discovery from functional to product: a peer published in DISCOVERY's minimized public shape minus its imagery, search preferences that are about the person holding them rather than filters over a population and that LIVE asks USERS rather than copying, choosing somebody as a request to meet with an `accepted` state that admits both people are not here yet and that authorizes nothing, reactions as lines in the encounter that are moderatable like messages and never gifts, the presence sweep composed on the worker in a composition that can admit nobody — it existed and nothing had ever called it — and both surfaces rearranged as stages around the picture rather than documents with a camera in them |
| [ADR-0042](decisions/ADR-0042-live-surface-refinements.md) | Live as a finished surface on both consumer surfaces, found by looking rather than by asserting: a closed chat that was covering every control on a phone because a class beat the `hidden` attribute, a stage four pixels wider than the viewport at every phone width, an immersive frame that lost the cascade to a document's padding on a laptop, controls leaving the screen at twice the text size, an edge-to-edge Android window that is handed a keyboard inset rather than being resized, one measured floor every floating layer clears, a deterministic picture-in-picture on the web and a dragged one on the phone, the identity moved out of the middle of the picture on a wide window, Choose drawn the way Discover draws the same people, and `/live` added to the width matrix that would have caught the first of them |
| [ADR-0043](decisions/ADR-0043-livekit-transport-coins-and-paid-live-preferences.md) | The first RTC adapter that carries a packet, and the money that narrows who it carries: LiveKit behind the unchanged provider port with a room name derived as an HMAC of the committed idempotency key so it is deterministic for recovery and unguessable without the secret, `maxParticipants: 2` enforced by the provider rather than by this platform, provider-side token revocation through `revokeTokenTs`, a join authorization that names where to present a credential and is absent when nothing carries media, coins as a second owner-specific append-only journal because a coin has no ISO 4217 currency and a fake one would make every currency rule in BILLING's journal a lie about it, reserve-on-activation and capture-on-first-filtered-match and release-in-full-on-expiry as one deliberate charging rule, a paid region narrowing that is bought as a bounded window rather than made expressible in the free preference contract, a matcher in which paying can only make the candidate pool smaller and every safety predicate is asked identically, an Android acquisition port with no adapter because no credential exists to verify a purchase against, and the microphone permission that was correctly absent until something could carry audio |
| [ADR-0044](decisions/ADR-0044-declared-matching-categories-and-premium-preference-sets.md) | What a person says about themselves, what that lets somebody buy, and who sells it: an optional self-declared matching category collected in its own table with one writer and one membership-only reader so "never inferred" is a property of the shape rather than a promise, `undisclosed` and "never asked" both deliberately unmatchable so declining costs nothing, a premium selection that is a conjunction of declared preferences priced as the sum of what is in it from a server catalogue both surfaces render, a charged window that keeps narrowing to expiry rather than ending the instant it is paid for, a narrowing that holds from both sides of the pair so the person who paid is never handed exactly what they paid to avoid, widening free because it can only ask for less while anything else is a new window sold as one, `billing_offers.owner_type` so a platform sale credits no creator payable and appears in nobody's earnings, and an acquisition path where one real purchase mints coins once across five redeliveries, three racing retries, a replayed dispatch, and a reinstalled store token |
| [ADR-0045](decisions/ADR-0045-consumer-completion-doorways-keyboards-and-a-camera-that-is-off.md) | The consumer product walked as a person rather than audited as a surface: declared doorways per route so a creator opened from your own memberships, a conversation opened from a Live encounter and a wallet opened from the Live door all lead back where they were opened from, a Back control that pops history rather than growing it so the page it returns to keeps its scroll and its loaded pages, a dialog that holds one history entry so a phone's system Back closes the sheet instead of the page, one measured keyboard mechanism owned by the screen frame because every keyboard-avoiding view in an Android-only build was inert, a hardware Back that asks before abandoning the person on the other end of an encounter, a creator directory and a creator's own identity on the phone where both screens existed and nothing linked to them, spoken languages made editable because they gate matching and a paid preference, failures that are said rather than dressed as absences, a camera turned off rendered as off in the same session with the voice still arriving through an element that outlives the picture, and a browser run that owns its ports or refuses to run |
| [ADR-0046](decisions/ADR-0046-answering-the-one-star-review.md) | The product built against the reviews of the products it competes with rather than against its own design: a short-lived way back to somebody a random encounter has already ended with, so an abusive stranger who presses Next cannot take the only control that named them off the screen; reporting and blocking as one act with the block applied first, because the dangerous failure is somebody believing they are separated when they are not; two reporter reasons for the allegations that were being filed as harassment; SUPPORT as a domain that depends on no provider, no configuration, and nothing that can be switched off, because the one path a person uses when everything else has failed them must not itself be able to fail; and account closure that actually closes — every session revoked, every push registration retired, any encounter and its session ended, the pool left — while publishing `erasureScheduled: false` rather than claiming data has been destroyed under a retention schedule nobody has approved |
| [ADR-0047](decisions/ADR-0047-public-entry-and-organic-acquisition.md) | A way in, built for an owner with no acquisition budget: GROWTH as a domain that owns how somebody arrived and nothing that follows from it, with an account's origin as a primary key so a second attribution is impossible rather than refused and attribution accepted only on the request that created the account; an invitation code that authorises nothing, is one per account forever, and never says who sent it; public pages rendered on the server because a page that fills itself in after hydration is an empty document to a crawler, a link preview, and a dropped connection alike; indexability as a decision with refusal as its default, requiring both a production environment and a declared origin so a preview deployment cannot be indexed by accident; a canonical built from the path alone so a referral cannot split one page into one indexed copy per link ever shared; withdrawn creators and closed clubs answering 404 rather than a tombstone a search engine keeps; a scheduled live window that concentrates people and publishes no attendance figure of any kind, because nothing knows; and no referral reward at all, because signup alone is what every fraudulent referral scheme has ever been paid for |
| [ADR-0048](decisions/ADR-0048-operator-control-plane-and-composed-activity.md) | Platform Admin becomes something an operator can operate: OPERATIONS as a domain owning capability grants, operational controls, and an append-only operator action log, and owning no product activity at all — the activity stream is composed at read time from the rows each domain already writes, so a retried action produces one entry or none, retention follows each domain's own decision, and the stream can never disagree with the record it describes; every operator route naming the one capability that authorises it, as a parameter with no default, and all three refusals answering the same code so nobody learns which condition they failed; a control that the server actually obeys on the path it governs, written as a compare-and-set so two operators during an incident resolve deterministically, cached with the propagation bound published rather than described as instant, and pausing in a way that admits nobody new while ending nothing already running; every operator command writing an audit row including the ones that were refused; an operator read bounded to three concurrent queries rather than fanning out across the whole connection pool, because ADR-0019's admission bound counts requests and cannot see one screen taking twenty of them; and no rate, score, percentage, or “users online” anywhere on the operator surface, because a number nobody can check is worse than no number |
...
- Consumer Web bootstrap: Consumer Web surface, Design/Figma authority, ADR-0016, ADR-0027, ADR-0037, unaffected portions of ADR-0003, ADR-0004, ADR-0005, ADR-0009, ADR-0013, ADR-0014, ADR-0015.
- Consumer Mobile bootstrap: Consumer Mobile surface, notification/deep-link authority, Design/Figma authority, ADR-0016, ADR-0027, ADR-0030, ADR-0039, unaffected portions of ADR-0003, ADR-0004, ADR-0005, ADR-0009, ADR-0013, ADR-0014, ADR-0015.
...
- Creator Studio bootstrap: Creator Studio surface, creator/club domains and flows, Design/Figma authority, ADR-0016, ADR-0020, ADR-0028, ADR-0038, unaffected portions of ADR-0003, ADR-0004, ADR-0005, ADR-0009, ADR-0010, ADR-0013, ADR-0014, ADR-0015.
- Platform Admin bootstrap: Platform Admin surface/domain/flow, RBAC, operations authority, Design/Figma authority, ADR-0016, ADR-0017, ADR-0029, ADR-0034, unaffected portions of ADR-0003, ADR-0004, ADR-0005, ADR-0009, ADR-0013, ADR-0014, ADR-0015.
...
- Billing and payouts: monetisation, BILLING/PAYOUTS, money flow, payment flow/security/compliance/operations, provider eligibility, virtual gifting freeze report, ADR-0006, ADR-0007, ADR-0009, ADR-0011, ADR-0013, ADR-0019, ADR-0021, ADR-0032, ADR-0035.
- AI: complete AI authority path, owning tool domains, AI action flow, ADR-0002, ADR-0007, ADR-0009, ADR-0012, ADR-0013, ADR-0014, ADR-0033.
...
- Billing: phases, monetisation, BILLING, payment lifecycle/security, jobs/concurrency, compliance, finance operations, Admin approval, ADR-0021, ADR-0032, ADR-0035, virtual gifting freeze report.
...
- AI: every document in AI authority, AI action flow/integration docs, product phases/surface, provider/outbound/jobs/RBAC/privacy/testing/operations, ADR-0033 for the local/test suggestion baseline, and every domain/tool contract involved.
