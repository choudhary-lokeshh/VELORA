...
## Implemented local/test baseline

[ADR-0033](../decisions/ADR-0033-local-test-ai-suggestion-platform.md) implements the first bounded answer/draft-only slice. Immutable capability manifests route one registered text-suggestion task; the model registry admits only the evaluated local/test provider/model pair. The Elysia Gateway authenticates Consumer Web, Consumer Mobile, Creator Studio, or phishing-resistant Platform Admin callers; checks an exact capability/audience pair and current USERS/CREATORS owner predicate; reserves a durable per-actor daily budget; and runs one provider-neutral streaming request with cancellation, timeout, bounded pre-output retry, output-size enforcement, strict response parsing, and redacted correlation logging.

PostgreSQL stores version-pinned capability activation, run state, append-only events, usage/cost counts, and SHA-256 input/output digests. It stores no raw draft, context, prompt, message, moderation evidence, or completion. The deterministic `local-test` adapter is network-free and reports zero provider cost. Configuration defaults to `unavailable`; staging and production reject `local-test` and reject disabling the kill switch.

This baseline has no tools, async orchestration, memory, RAG, retrieval, provider fallback, or domain effect. Those architecture seams remain governed but unimplemented. A live adapter remains `DEFER UNTIL PROVIDER INTEGRATION` and requires capability-specific evaluation and provider/legal/privacy/security approval.

No AI product capability is V1. Moderation assistance is Phase 2; approved consumer, creator, and Admin assistance is Phase 3; broader autonomous capabilities are Future / Moonshot. [ADR-0012](../decisions/ADR-0012-ai-platform-runtime.md), with backend mechanics superseded by [ADR-0016](../decisions/ADR-0016-bun-elysia-redis-bullmq-backend.md), locks Elysia API admission, explicit bounded TypeScript orchestration, version-controlled prompt/JSON Schema releases, PostgreSQL run/registry/budget metadata, BullMQ async work, and OpenTelemetry API seams. Provider/model/embedding/hosting routes are `DEFER UNTIL PROVIDER INTEGRATION`; capability-specific evaluation/routing policy, durable memory, and RAG/vector implementation are `DECISION REQUIRED BEFORE FEATURE`.
