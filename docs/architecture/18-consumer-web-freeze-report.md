...
**The surface was driven again and refined under [ADR-0037](../decisions/ADR-0037-consumer-web-experience-refinements.md).** Nothing below about architecture, contracts, or capability changed. What changed is presentation and interaction: a page no longer prints its own name twice on a phone, a Back names the destination it returns to where the navigation has a word for it, the creator listing and a status row no longer push the document sideways at 200 % text, the section switch behaves the way `role="tablist"` promises on a keyboard, the introductions group became an address that opens where the work is, eleven hand-built sections became one `Section` primitive, the gift history draws the gift and says what each state means, a club refusal names its gate, an unsent message has one copy and one safe way to send it, and the writing assist is folded away inside a conversation. The responsive evidence below is amended: the 200 %-text assertion now runs over every product route rather than one.
