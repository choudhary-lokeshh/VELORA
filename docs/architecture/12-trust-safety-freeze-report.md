...
- **No depicted-person verifier.** `SAFETY_DEPICTED_PERSON_VERIFIER` defaults to `unavailable`, which refuses every verification, and staging and production reject any other value. Nobody can be recorded as a verified depicted adult, so no depiction can be cleared.
