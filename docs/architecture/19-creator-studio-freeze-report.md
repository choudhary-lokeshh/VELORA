...
**The workspace was driven again and refined under [ADR-0038](../decisions/ADR-0038-creator-studio-workspace-refinements.md).** Nothing below about architecture, contracts, or capability changed. What changed is presentation and interaction: the tab bar keeps all five destinations on a 320 px screen at 200 % text where it used to lose Money entirely, a button label, a badge and a row no longer push the document sideways at that size, a page no longer prints its own name twice on a phone, a Back names the destination it returns to where the navigation has a word for it, which slice of the catalog is being read became an address that survives a reload, the filter strip behaves the way `role="tablist"` promises on a keyboard and names the region it changes, and the received-gifts row draws the gift, dates it like every other date in the workspace, and tells a failure apart from a settlement. The responsive evidence below is amended: the 200 %-text assertion was applying its style tag before navigating and therefore measured ordinary text on every route it claimed to zoom; it now runs at 320 px over every route, with the tab bar asserted separately.
