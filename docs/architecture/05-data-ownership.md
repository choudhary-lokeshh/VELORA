...
- AUTH owns authentication identity, its `auth_`-prefixed tables, and its own security events; USERS owns profile; CREATORS owns creator business identity; PRIVATE CLUBS owns entitlement; BILLING owns customer money state; PAYOUTS owns disbursement state; Trust & Safety owns block/enforcement state.
