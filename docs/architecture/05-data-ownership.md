...
- AUTH owns authentication identity, its `auth_`-prefixed tables, and its own security events; USERS owns the consumer account, profile, self-declared adult status, and its `users_`-prefixed tables; IDENTITY ASSURANCE owns verified assurance subjects/attempts/evidence/provider receipts/reconciliation under `identity_`, linked to owner domains by opaque references only; CREATORS owns creator business identity and its `creators_`-prefixed tables, linked to AUTH by an opaque account identifier alone; PRIVATE CLUBS owns the creator content catalog, club membership, and entitlement under its `clubs_`-prefixed tables, referencing a creator by opaque identifier alone; BILLING owns customer money state; PAYOUTS owns disbursement state; GROWTH owns invitations, signup attribution, acquisition counts, and scheduled live windows under `growth_`, referencing accounts by opaque identifier alone and owning nothing that follows from arriving; OPERATIONS owns operator capability grants, operational control values, and the operator action log under `operations_`, referencing operators by opaque AUTH account reference and subjects by whatever kind the action named, in both cases with no foreign key; Trust & Safety owns block/enforcement, depicted-person relationship, and consent state.
...
GROWTH's tables record how somebody arrived and nothing that follows from arriving. `growth_invites` is unique on the owner, so one account has one link forever — minting a second would silently break every link that person had already sent. `growth_signup_attributions` is keyed on the account itself, which is what makes "exactly one origin, forever" a property of the schema rather than a rule a service remembers; a CHECK refuses an inviter equal to the invited account. `growth_acquisition_events` has a closed vocabulary and **no payload column**, so a message, a profile field, or a token cannot end up in it, and there is no address, referer, user agent, IP address, or session identifier anywhere in the domain. `growth_live_windows` stores two instants and derives its state on every read, because a stored state is wrong for exactly as long as it takes a job to update it. Every account reference here is an opaque USERS identifier with no foreign key, so an account disappearing never takes the record of where somebody came from with it.

OPERATIONS' three tables are deliberately small and none of them grows with the product. `operations_controls` is one row per switch keyed on the switch, with a version column that makes every write a compare-and-set — the second of two operators looking at the same screen is refused rather than silently overwriting a change they never saw. `operations_operator_grants` permits one live grant per operator through a partial unique index; a revoked grant keeps its row, because it is the evidence somebody held a capability during the window an incident happened in. `operations_operator_actions` is append-only in the code above it: there is no update, no delete, and no method taking an action identifier anywhere in the domain. It has no payload column — `previousState` and `requestedState` are short bounded projections such as `enabled` or a role name, and there is nowhere to put a message, a token, or a body.

**There is no activity, telemetry, or metric table anywhere in this repository.** The operator activity stream is composed at read time from the rows domains already write, so a retried action produces one entry or none, retention follows each domain's own decision automatically, and a stream entry can never disagree with the record it describes. [ADR-0048](../decisions/ADR-0048-operator-control-plane-and-composed-activity.md) records the argument and the cost.

`DECISION REQUIRED / LEGAL REVIEW REQUIRED`: retention durations for all of the above, including the operator action log. No duration is invented here, and because no rule depends on a row being physically gone, an approved one can be applied later as a deletion pass without changing behaviour.
