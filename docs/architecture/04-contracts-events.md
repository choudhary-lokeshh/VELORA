...
- Identity events contain only opaque subject/evidence references, evidence class, normalized lifecycle fact, policy version, validity metadata, and correlation. They never contain provider callback bodies, hosted URLs, document/selfie/biometric data, exact date of birth, tax or bank data, or an owner-domain authorization decision.
...
Identity provider callbacks are not domain events. Signature/account/environment verification happens against the exact raw body before a minimized receipt enters the Identity inbox. Provider event identity is unique per provider/account/environment; duplicate verified delivery returns the same accepted receipt, while invalid or unverifiable input creates no inbox row. Reconciliation compares provider state with durable platform state but can only append a finding or a valid superseding evidence fact.
...
V1 uses Zod 4 schemas, generated OpenAPI 3.1/TypeScript clients, PostgreSQL transactional outbox/inbox, and BullMQ execution under [ADR-0016](../decisions/ADR-0016-bun-elysia-redis-bullmq-backend.md) plus the unaffected contract/outbox portions of ADR-0005 and ADR-0007. PostgreSQL owner state and the outbox fact are atomic; publication and job handling are idempotent and reconcilable. An external broker is deferred until measured throughput, fan-out, retention, or extraction requires it. See [API contracts](../engineering/01-api-contracts.md), [Identity Assurance](../domains/identity-assurance.md), [AI capabilities/tools](../ai/02-ai-capabilities-tools.md), [jobs/idempotency](../engineering/03-jobs-idempotency-concurrency.md), [analytics](../domains/analytics.md).
