...
Reconciliation records bounded, durable findings for missing references, provider/platform state drift, stuck attempts, expiry, callback gaps, and deletion/retention obligations. A finding is evidence of drift, not permission to overwrite owner-domain truth. Its scheduling marker is technical metadata only: it never changes an attempt lifecycle, an evidence fact, an owner predicate, or a privacy decision.
...
`IdentityReverificationPolicy` is a server-only, read-only comparison between a minimized current evidence fact and the currently evaluated policy. It reports only `current`, `no_current_grant`, `policy_unknown`, `policy_blocked`, or `reverification_due` for expiry, policy-version, or requirement change. It neither authorizes a product capability nor starts, creates, revokes, or expires an attempt/evidence record. No V1 owner predicate or product surface consumes this assessment; an owner may do so only after the applicable phase, legal policy, and authorization design are approved.
...
- Cross-subject and cross-Creator reads are refused identically to absent records. Admin exact-reference reads require the Platform Admin audience, current Admin authentication assurance, and a one-time ADR-0017 exact-action authorization bound to both the identity subject and the executing Admin session; they are not a mutation or approval path. No public route issues that authorization, and its use does not settle the still-unresolved granular Admin permission matrix.
...
Implementation status: migration `0046_identity-assurance-foundation` establishes the Identity-owned subjects, attempts, append-only evidence, verified-receipt inbox, reconciliation findings, and outbox. The internal owner-authorized start contract accepts only the documented owner/purpose pairing, selects policy and provider only from validated server configuration, commits a stable attempt before provider I/O, converges concurrent duplicates, recovers ambiguous creates by the same provider key, validates provider subject/reference binding, and never persists a hosted URL. `POST /v1/identity/provider-events` authenticates exact raw bytes, stores only a minimized verified receipt, and acknowledges exact duplicates identically; a recurring worker claims receipts under PostgreSQL leases, retrieves current provider truth outside transactions, applies append-only evidence plus its outbox fact atomically, and bounds retry/dead-letter behavior. Migration `0047_identity-assurance-users-cutover` moves legacy verified-adult history into AUTH-principal Identity subjects/attempts/evidence, retires mixed USERS storage, and publishes a minimized current-adult-assurance reader consumed by USERS. Migration `0050_identity-safety-cutover` moves legacy depicted-person identity/adult provider facts into two IDENTITY evidence classes, replaces the SAFETY record with an opaque Identity subject reference, and preserves participant/consent linkage under transactional count and rollback assertions. Migration `0051_identity-reconciliation-scheduling` adds an indexed technical scan marker and preserves terminal-attempt immutability except for that marker. The recurring reconciliation worker takes a bounded due page under PostgreSQL row locks, then reads the configured provider outside a transaction. It records a normalized finding before it reapplies a missing provider state through the same append-only evidence/outbox path as a verified callback; `created` attempts are recorded as stuck and never started autonomously, while a later revocation or expiry of a succeeded attempt becomes a successor evidence fact. Retrieval failure is a redacted operational failure, not a fabricated result. Reconciliation does not execute deletion, retention, export, correction, provider erasure, or manual repair: those remain unavailable pending the documented legal/privacy and provider decisions. IDENTITY also publishes coarse Creator-identity and commercial-KYC readers. The read-only re-verification comparison detects expiry and current-policy changes without creating a workflow or altering owner predicates. The implemented Admin operations projection exposes only identifier-free aggregate health and an exact subject projection; the latter consumes a one-time ADR-0017 authorization bound to the subject and executing Admin session and never becomes a search, list, export, mutation, or general-purpose Admin permission. Migration `0052_identity-operations-scale` narrows the attempt history index to `(subject_id, sequence)` so the exact-reference read pages a subject's mixed-purpose history in lifecycle order without sorting it; a real PostgreSQL suite asserts the query plans behind subject resolution, attempt history, current-evidence derivation, and finding history at the documented V1 scale boundary, and asserts that the Admin projection stays bounded and truncation-marked over retained history. Migration `0053_identity-evidence-root` closes what a hostile read of the finished domain found: the chain was enforced in its middle but not at its root, so two roots of one class could both be current tips. An adversarial suite additionally proves that no evidence class answers for another, that a grant is not current at the instant it expires, that a revocation reads as a revocation, that a delayed provider success does not outrank the attempt recorded after it, that a re-verification in flight withholds the superseded answer rather than serving it, that a depicted-person read answers only for the exact participant and subject pairing and is otherwise indistinguishable from an absent record, that an owner reference never resolves across owner domains, and that a published decision carries no provider or evidence handle. No Creator predicate, Phase 3 payout readiness, or verification surface is enabled. `unavailable` plus unpublished policy remain the defaults; the network-free `local-test` provider and policy are rejected in staging and production. No public start route, real provider, or product surface is enabled.
...
See [verification flow](../flows/identity-assurance-verification.md), [threat model](../security/11-identity-verification-threat-model.md), [provider eligibility](../compliance/09-identity-verification-provider-eligibility.md), [operations](../operations/07-identity-verification-operations.md), [the freeze report](../architecture/14-identity-freeze-report.md), and [ADR-0024](../decisions/ADR-0024-identity-assurance-architecture.md).
