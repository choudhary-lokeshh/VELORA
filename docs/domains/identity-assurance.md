...
Implementation status: migration `0046_identity-assurance-foundation` establishes the Identity-owned subjects, attempts, append-only evidence, verified-receipt inbox, reconciliation findings, and outbox. The internal owner-authorized start contract selects policy and provider only from validated server configuration, commits a stable attempt before provider I/O, converges concurrent duplicates, recovers ambiguous creates by the same provider key, validates provider subject/reference binding, and never persists a hosted URL. `POST /v1/identity/provider-events` now authenticates exact raw bytes, stores only a minimized verified receipt, and acknowledges exact duplicates identically; a recurring worker claims receipts under PostgreSQL leases, retrieves current provider truth outside transactions, applies append-only evidence plus its outbox fact atomically, and bounds retry/dead-letter behavior. `unavailable` plus unpublished policy remain the defaults; the network-free `local-test` provider and policy are rejected in staging and production. No public start route, owner migration, reconciliation service, Admin Identity route, real provider, or product surface is enabled. Existing USERS and SAFETY compatibility storage remains authoritative until its separately tested backfill/cutover migration lands.
