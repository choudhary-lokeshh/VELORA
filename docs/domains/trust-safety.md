...
Every piece of this vertical is tested where it lives, and until now nothing tested the *chain*. A suite walks one story once: somebody reports somebody, a case opens carrying that report as evidence, a reviewer claims it and adds a note, a decision cites both and restricts the account through USERS, the person it happened to reads a statement of reasons and contests it, a reviewer upholds the complaint, and the reversal is a second decision that leaves the first byte-for-byte as written. The account is active again, the surface stops saying otherwise, and the counts afterwards are exact — two decisions, two enforcement records, two pieces of evidence, one report, one complaint. Nothing was erased on the way, including the decision that turned out to be wrong.

The last step of that walk re-reads all three consumer surfaces and asserts that the reporter's narrative, the reviewer's note, the appellant's statement, and the reporter's identifier appear in none of them. A vertical whose parts each pass and whose chain does not is a vertical that passes its tests and fails its user.

**The review seam holds a subject advisory lock for the whole of a decision** — a lock taken inside a transaction, held across several statements, released only by the commit. That is the same shape as the pair lock the Bun.SQL pool defect used to lose connections to. It was already proven not to deadlock, but on the harness connection rather than on a pool, so nothing showed what contention on it costs a deployed process. It is now driven on the real `DatabaseService` at production sizes: four cases decided three times each, all twelve at once, so the subject lock is contended within each case and uncontended between them. Exactly one settlement per case, every subject restricted, and the pool holds the connections it opened — no stalled transaction, no lost backend, no deadlock.
