...
Three conditions guard every route and are never collapsed. The audience must be Platform Admin: a consumer session and a Creator Studio session are refused before any lookup happens on their behalf. The assurance must be phishing-resistant and recent, which [ADR-0017](../decisions/ADR-0017-auth-session-recovery-security-policy.md) requires for privileged access — so with no approved verifier these operations are unreachable in a deployed environment rather than degraded to something weaker. And since [ADR-0048](../decisions/ADR-0048-operator-control-plane-and-composed-activity.md) the operator must hold the capability the route names: `AdminContextResolver.resolve` takes it as a parameter with no default, so a new operator route cannot be written without deciding what authorises it. All three refuse with the same status and code, because which condition failed is not a caller's business.
...
V1 minimum support/moderation/enforcement/country-flag/audit/health tools. Finance/payout/dispute workflows phase with owning domains. Events: role, approval, privileged read/write, configuration change. Privileged session, step-up assurance age, exact-action approval binding, privileged recovery, and break-glass semantics follow [ADR-0017](../decisions/ADR-0017-auth-session-recovery-security-policy.md). The granular permission matrix is now implemented rather than open: twenty-two capabilities and seven roles in [OPERATIONS](operations.md), with roles as convenience sets over capabilities so adding one can never widen what a route admits. `DECISION REQUIRED`: approval thresholds, break-glass implementation, audit retention, and who holds the first operator grant in a deployed environment. See [Platform Admin product](../product/04-platform-admin.md), [Platform Admin surface](../surfaces/04-platform-admin.md), [admin operations](../flows/admin-operations.md), [operations authority](../DOCS_INDEX.md#operations-authority), [RBAC](../security/02-access-control-rbac.md).
