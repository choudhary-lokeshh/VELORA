 ...
 *
 * `matchingGender` is assigned by position in a repeating cycle — woman, man,
 * non-binary, prefer-not-to-say, and then nobody at all — rather than chosen to
 * suit each name. That is deliberate and it is the point: VELORA never infers a
 * declaration from a name, and a fixture set where every declaration agreed
 * with the name beside it would let an inference bug pass every local walk.
 * Every fifth person has no declaration, because "never asked" is a real state
 * a paid filter has to handle and is the state every existing account is in.
 */
