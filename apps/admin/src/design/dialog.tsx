 ...
 *
 * On this surface it is where every high-impact operation is confirmed, so the
 * exact target and the exact effect are parameters rather than something a
 * caller may leave to a colour.
 *
...
export function Dialog({
  ...
  return (
    <>
      ...
      */}
      <div className="a-scrim" onClick={close} />
      <div
        ...
        tabIndex={-1}
      >
        <span className="a-sheet-grip" />
        {title === undefined ? null : (
          <div className="a-dialog__head">
            <h2 className="a-heading" id={headingId}>
              {title}
            </h2>
            <button
              aria-label="Close"
              className="a-dialog__close a-icon-btn a-icon-btn--sm"
...
export function ConfirmDialog({
  ...
  return (
    <Dialog onClose={onCancel} testId={testId} title={title}>
      <div className="a-small a-muted a-stack a-stack--3">{children}</div>
