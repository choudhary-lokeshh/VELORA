import type { CreatorReceivedGift } from '@velora/creator-client';

export type GiftVisual = CreatorReceivedGift['gift']['visual'];

/**
 * One silhouette per gift, on the same 24-unit grid every other mark uses.
 *
 * `visual` is in the contract and this screen drew the first letter of the
 * gift's name instead — so a creator read "R" where the person who sent it, and
 * paid for it, saw a rose. The same gift is now the same shape on both sides of
 * the transaction.
 *
 * Studio keeps its own copy rather than importing the consumer surface's.
 * `AGENTS.md` holds the surfaces apart and Studio already owns its whole mark
 * set; a shared component here would be one import that quietly couples a
 * creator workspace to a consumer feed. The geometry is the contract's, not
 * either surface's.
 */
export const giftShapes: Readonly<Record<GiftVisual, string>> = {
 ...
 * it is hidden from assistive technology rather than given a name that would be
 * read out twice.
 */
export function GiftArt({
  className = 's-gift-art',
  visual,
}: {
  readonly className?: string;
  readonly visual: GiftVisual;
}) {
  return (
