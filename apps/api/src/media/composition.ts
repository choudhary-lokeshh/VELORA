...
> = {
  [localTestMediaStorage]: (config) => {
    ...
    // Configuration defaults this from the process's own bind address, so it is
    // always a well-formed origin by the time composition reads it.
    return new LocalTestMediaStorage({
      baseUrl: config.VELORA_API_BASE_URL,
      directory,
      signingKey,
    });
  },
